...
void saveCfgSystem(){
 ...
 private void jbInit() throws Exception  {
    ...
    this.setTitle("AGGA 1.1 - ASCRE GSM-GPRS analysis 1.1");
    ...
    jMenuHelpAbout.setText("О программе AGGA 1.1");
    ...
    outAllDataGPRS.addActionListener(new Frame1_outAllDataGPRS_actionAdapter(this));
...
if (exCfgSystem.tranGSM.isEmpty())
{
      if (lang == true)
      {
        JOptionPane.showMessageDialog(this,"There are no GSM channels. Please, set structure of the system.","Attention!",JOptionPane.WARNING_MESSAGE);
        exCfgSystem.show();
      }
      else {
...
exCfgSystem.tranGSM.isEmpty()==true & exCfgSystem.tranGPRS.isEmpty()==true){
      if (lang == true)
      {
        JOptionPane.showMessageDialog(this,"There are no communication channels. Please, set structure of the system.","Attention!",JOptionPane.WARNING_MESSAGE);
      }
      else {
...
if (Engine.isRunning())
{
 Engine.stop();
  ...
  else
  {
      if (lang == true)
    {
      JOptionPane.showMessageDialog(this, "The simulation hasn't been launched yet!",
                                    "Attention!", JOptionPane.WARNING_MESSAGE);
    }
    else
    {
        JOptionPane.showMessageDialog(this, "Процесс моделирования не запущен!",
  ...
  void bRestart_actionPerformed(ActionEvent e) {
    ...
    if (lang == true)
    {
      n = JOptionPane.showConfirmDialog(Engine.getAnimation(),
                                            "Would you like to save current configuration of the system?",
                                            "Attention!",JOptionPane.YES_NO_CANCEL_OPTION,JOptionPane.QUESTION_MESSAGE);

    }
    else
    {
       n = JOptionPane.showConfirmDialog(Engine.getAnimation(),
                                              "Сохранить текущую конфигурацию ситемы?",
                                              "Внимание!",JOptionPane.YES_NO_CANCEL_OPTION,JOptionPane.QUESTION_MESSAGE);
    }
      switch(n){
  ...
  void bCfgGPRS_actionPerformed(ActionEvent e) {
    if (exCfgSystem.tranGPRS.isEmpty())
    {
      if (lang == true)
      {
        JOptionPane.showMessageDialog(this,"There are no GPRS channels. Please, set structure of the system.","Attention!",JOptionPane.WARNING_MESSAGE);
        exCfgSystem.show();
      }
      else {
  ...
  void bShowGrGPRS_actionPerformed(ActionEvent e) {
    if (transferDataGPRS.size() == 0 ) {
      ...
      if (lang == true)
      {
        graphGPRS  =  new  graphGPRSData("Total data volume transmitted through GPRS channels",
                                         transferDataGPRS,
                                         "Data volume for each channel",
                                         "Data volume, Kb",
                                         "GPRS channel"
                                         );
      }
      ...
      {
        graphGPRS  =  new  graphGPRSData("Объём переданных данных по каналам GPRS",
                                         transferDataGPRS,
                                         "Объём переданных данных по каждому каналу",
                                         "Объём информации, Кбайт",
                                         "Канал GPRS"
                                         );
      }
    ...
    if (exCfgSystem.tranGSM.isEmpty())
   {
     if (lang == true)
     {
       JOptionPane.showMessageDialog(this,
           "There are no GSM channels. Please, set structure of the system.",
                                     "Attention!", JOptionPane.WARNING_MESSAGE);
       exCfgSystem.show();
       ...
       JOptionPane.showMessageDialog(this,
           "Отсутствуют GSM каналы!Сначала задайте структуру системы.",
                                     "Внимание!", JOptionPane.WARNING_MESSAGE);
       exCfgSystem.show();
     }
...
exCfgSystem.show();
    ...
    if (transferVrGPRS.size() == 0) {
     ...
     else
     {
       JOptionPane.showMessageDialog(this, "Данных по GPRS каналу нет!",
        ...
        if (lang == true)
      {
        graphGPRSVr  =  new  graphGPRSTime("Mean polling time through GPRS channels",
                                         transferVrGPRS,
                                         "Mean polling time for each channel",
                                         "Time, minutes",
                                         "GPRS channel"
                                         );
      }
      ...
      {
        graphGPRSVr  =  new  graphGPRSTime("Среднее время опроса по каждому каналу",
                                         transferVrGPRS,
                                         "Среднее значение времени опроса по каждому каналу",
                                         "Время, минут",
                                         "Канал GPRS"
                                         );
      }
    ...
    else
    {
      ...
      if (lang == true)
      {
        graphGPRSSET = new graphCounterGPRS(
            "Mean polling time through GPRS channels",
            transferTimeSETGPRS, transferTimeMIRGPRS,
            "Mean polling time a counter in each channel",
            "Time, minutes",
            "GPRS channel",
            "SET-4ТМ",
            "MIR С-01"
        ...
        graphGPRSSET = new graphCounterGPRS(
            ...
            transferTimeSETGPRS, transferTimeMIRGPRS,
            "Среднее значение времени опроса одного счетчика в канале",
            "Время, минут",
            "Канал GPRS",
            "CЭТ-4ТМ",
            "МИР С-01"
      ...
      graphGPRSSET.pack(); RefineryUtilities.centerFrameOnScreen(graphGPRSSET); graphGPRSSET.setVisible(true);
   }
  }
    ...
    if (transferTimeSETGSM.size() == 0 & transferTimeMIRGSM.size()==0) {
      if (lang == true)
      {
        ...
        JOptionPane.showMessageDialog(this, "Данных по GSM каналу нет!",
             ...
             if (lang == true)
             {
               graphGSMSET = new graphCounterGSM(
                   "Mean polling time through GSM CSD channels",
                   transferTimeSETGSM,transferTimeMIRGSM,
                   "Mean polling time a counter in each channel",
                   "Time, minutes",
                   "GSM CSD channel",
                   "SET-4ТМ",
    ...
    if (transferVrGSM.size() == 0) {
      if (lang == true)
      {
        ...
        JOptionPane.showMessageDialog(this, "Данных по GSM каналу нет!",
         ...
         if (lang == true)
         {
           graphGSMVr = new graphGSMTime(
               "Mean polling time of GSM CSD channels",
               transferVrGSM,transferTimeUstGSM,transferTimeEndGSM,
               "Mean polling time in each channel",
               "Time, minutes",
               "GSM CSD channel",
               "Connection time",
               "Transmission time",
           ...
           graphGSMVr = new graphGSMTime(
               "Среднее время опроса по каждому каналу",
               transferVrGSM,transferTimeUstGSM,transferTimeEndGSM,
               "Среднее значение времени опроса по каждому каналу",
               "Время, минут",
               "Канал GSM CSD",
  ...
  void jMenuItem4_actionPerformed(ActionEvent e) {
    if (exCfgSystem.tranGSM.isEmpty())
    {
      ...
      if (lang == true)
      {
        ...
        JOptionPane.showMessageDialog(this,"Отсутствуют канала связи! Задайте структуру системы.","Внимание!",JOptionPane.WARNING_MESSAGE);
        exCfgSystem.show();
      }
    }
        else
  ...
  void outAllDataGPRS_actionPerformed(ActionEvent e) {
    if (lang == true)
    {
      JOptionPane.showMessageDialog(this,"There is no need to input anything here","Attention!",JOptionPane.WARNING_MESSAGE);
    }
    else {
  ...
  void bOption1_actionPerformed(ActionEvent e) {
    if (lang == false)
    {
      ...
      exCfgSystem.jLabelSET.setText("SEТ-4ТМ");
      ...
      exCfgSystem.msgYesNo = "Delete GSM(Yes) or GPRS(No) channel?";
      exCfgSystem.msgNoGSM = "There is no any GSM channel!";
      exCfgSystem.msgNoGPRS = "There is no any GPRS channel!";
      exCfgSystem.msgError = "Error!";
    //--------------------------------------------------------------//
      ...
      Object[] dataIntervaleng = {new String("Once a day"),
                 new String("60 minutes"),
                 new String("30 minutes"),
                 new String("15 minutes"),
                 new String("5 minutes"),
                 new String("3 minutes"),
      };
      exDcfgMod.jComboBox1.removeAllItems();
      DefaultComboBoxModel model = new DefaultComboBoxModel( dataIntervaleng );
      exDcfgMod.jComboBox1.setModel( model );
      //--------------------------------------------------------------//
      ...
      exCfgCounter.selectSET.setLabel("SET-4ТМ");
      ...
      exAnimation.jLabel1.setText("The simulation is running");
      exAnimation.jButton1.setText("Cancel");
      exAnimation.jLabel3.setText("Please, wait!");
      exAnimation.jLabel4.setText("Done:");
      ...
      this.jMenuHelpAbout.setText("About AGGA 1.1");

      dlg.label4.setText("The application serves for the analysis of automated ");
      dlg.label1.setText("monitoring systems of energetic objects built on the ");
      dlg.label5.setText("communication channels of celluar networks");
      ...
      jSystemStructure.setText("Structure of the system");
      ...
      jLabel5.setText("SEТ4ТМ - ");
      jLabel6.setText("MIR С-01 - ");
      jLabel7.setText("SEТ4ТМ - ");
    ...
    else
    {
        ...
        exCfgSystem.jLabelSET.setText("СЭТ-4ТМ");
        ...
        exCfgSystem.msgYesNo = "Удалить GSM(Да) или GPRS(Нет) канал?";
        exCfgSystem.msgError = "Ошибка!";
        exCfgSystem.msgNoGSM = "Канал GSM отсутствует!";
        exCfgSystem.msgNoGPRS = "Канал GPRS отсутствует!";
        //--------------------------------------------------------------//
        ...
        Object[] dataIntervalrus = {new String("Раз в сутки"),
                         new String("60 минут"),
                         new String("30 минут"),
                         new String("15 минут"),
                         new String("5 минут"),
        ...
        exDcfgMod.jComboBox1.removeAllItems();
        DefaultComboBoxModel model = new DefaultComboBoxModel(dataIntervalrus);
        exDcfgMod.jComboBox1.setModel(model);
        //--------------------------------------------------------------//
        ...
        exCfgCounter.selectSET.setLabel("СЭТ-4ТМ");
        ...
        exAnimation.jLabel1.setText("Идёт процесс моделирования!");
        exAnimation.jButton1.setText("Отмена");
        exAnimation.jLabel3.setText("Подождите!");
        exAnimation.jLabel4.setText("Выполнено:");
        ...
        this.jMenuHelpAbout.setText("О программе AGGA 1.1");
        dlg.label1.setText("АСКУЭ, построенных на базе технологий");
        dlg.label4.setText("Программа предназначена для анализ систем ");
        dlg.label5.setText("передачи данных мобильных сетей связи");
...
class Frame1_outAllDataGPRS_actionAdapter implements java.awt.event.ActionListener {
  Frame1 adaptee;

  Frame1_outAllDataGPRS_actionAdapter(Frame1 adaptee) {
    this.adaptee = adaptee;
  }
  public void actionPerformed(ActionEvent e) {
    adaptee.outAllDataGPRS_actionPerformed(e);
  }
}
